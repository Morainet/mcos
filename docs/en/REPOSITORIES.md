...
| Package | `com.morainet.mcos.android` — root (`CompositionRoot`/`AppDeps`, `RuntimeBootstrap`, `ScheduleAlarmReceiver`, `BootReceiver`, `DynamicPluginLoader`, `MarketplacePluginFactory`, `PluginPermissionBootstrap`, `TrustAnchors`, `McosHostApp`, `McpServerController` + `McpServerBridge` port + result types, `TriggerMaintenance`, `MarketplaceTrust`) + `host` (`AndroidHostServices`, `ActivityResultBridge`, `RuntimePermissionBridge`, `AlarmManagerWakeScheduler`, `AndroidLlmHttpTransport`, `AndroidMarketplaceHttpTransport`) + `host.isolation` (pure-Kotlin isolation RPC, 08 §8: `IsolationChannel`/`IsolationOps`/`IsolationCodec` wire codec, `BinderIdentityPolicy`, `IsolatedFacadeServer`, `IsolatedHostServicesProxy`, `TransportIsolationHost`, `IsolatedPluginRunner`, the Binder byte transport (`BinderWire` framing, `WirePipe`/`PipeIsolationChannel`, `WireService` serving cores, `BinderWirePipe`/`FacadeBinderEndpoint`/`InvokeBinderEndpoint`, `IsolatedPluginProcessService` in `:mcos_plugin`, `BinderIsolationHost`, `StagedArtifactResolver`) — opt-in activation seam landed (item 44, `processIsolation` flag, default off) plus manifest-only registration (item 45: under the flag the install pipeline registers from the decoded `plugin.json` — `McosPackage.readPluginManifest` feeding `PluginLoader.loadManifest` → `CommandRegistry.registerManifest` — so plugin dex loads only in `:mcos_plugin`); on-device verification pending) |
| Role | Everything an Android app needs to host the MCOS runtime **with no UI attached**: the composition root (production security posture, injectable built-ins via `CompositionRoot.create(context, builtIns)`), headless process bootstrap + durable-schedule re-arm receivers, the `McosHostApp` seam receivers use to reach the host's `AppDeps`, activity-result / runtime-permission bridges, the HTTP transport adapters, and the **host controllers** behind a host's own UI — MCP server management (`McpServerController` drives persistence/secrets/enable-disable through the runtime install pipeline at builtin trust, with discovery injected via the `McpServerBridge` port so the library stays free of any MCP client dependency), the uninstall disarm-sweep (`TriggerMaintenance`), revocation refresh (`MarketplaceTrust`), and install-time activation (`PluginPermissionBootstrap.activate`) — plus the **isolation RPC layer** (08 §8): the pure-Kotlin halves of the plugin-process boundary — facade server *and* the `IsolatedPluginRunner` that executes invocations inside the plugin process — JVM-testable end to end (real Executor → channel → runner → proxy → facade over the host services), carried over Binder in production behind the opt-in `processIsolation` flag (item 44; default off until on-device verification), with registration manifest-only under the flag (item 45) so the main process loads no plugin dex at all. The library manifest carries all host permissions, receivers, and the FileProvider, merged into any integrating app. |
...
| Role | `sys.notify` / `sys.share` / `sys.intent.start` (§12.3 Intent launching, carrying §12.6's schema-constrained extras and the published well-known-intent allowlist) commands. |
...
| Role | `deeplink.open` / `appfn.invoke` commands — the bridge to other apps' surfaces. `deeplink.open` launches an `ACTION_VIEW` deep link, and `appfn.invoke` calls an App Function published by another package via the §12.5 command-id encoding (`AppFunctionIds`). The Intent command itself belongs to the `sys` plugin (`sys.intent.start`, §12.3/§12.6, with the published extras allowlist); 10-roadmap §5.4's `intent.start` resolves there as an **alias**, so this module holds no second copy of the §12.6 rule. Standalone like iot/mcp: a host wires it explicitly, so it is not in the `android-sdk` default built-in set. |
| Depends on | `api(project(":mcos-sdk"))`; serialization-json, coroutines-core |
| Stack | Kotlin/JVM · JDK 17 |
