...
> ✅ **As-built (post item-46 alignment, item-47 hardening):** the signatures above are the live `mcos-sdk` interface — byte-valued `get`/`put` plus the `keys()` enumeration (the secret-hygiene surface uninstall cleanup and rotation need). The Android host implementation (`AndroidSecureStore`) now seals every value at rest with AES-256-GCM under a key generated inside — and never leaving — AndroidKeyStore (`[0x01][12-byte IV][ciphertext‖tag]` envelopes, Base64 in app-private `SharedPreferences`); a fresh random IV per write means no ciphertext-equality oracle, and tampering with a sealed value surfaces as a loud failure, never a silent null. Values written before the hardening read back as-is and re-seal on their next write — no migration step. Honest boundary: the Keystore/SharedPreferences glue itself is on-device verification surface; the envelope framing and stored-string codec are JVM-unit-tested. `{{secret.*}}` template resolution ([08 §9.2](./08-security.md)) consumes the byte values textually, unchanged by the wrapping.
