...
> **As-built (item 37, slice 2 of 3):** checks 2–3 are live in the JVM runtime as `StampScopedNetService` — the Stage-4 `NetService` handed to every non-`BUILTIN` plugin verifies stamp presence, signature (the configured signer; only the named `TrustingAuthStampSigner` waives), TTL, and `network.<domain>` scope — via the shared `DomainGlob` matcher, identical to Stage 6.5 egress matching — before any request leaves the runtime. Denials surface as `PERMISSION_DENIED` with `details.reason` of `stamp_scope_mismatch` (or `stamp_missing` / `stamp_signature_invalid` / `stamp_expired` / `invalid_url`) and land in the Stage-10 audit step. The `FileService` `command.*` scope awaits a grant-model extension (the kernel mints no `command.*` scopes today). Until the process boundary exists, an in-process plugin can still bypass the facade with its own HTTP client (the [§12](#12-network-egress-control) MVP limitation) — this gate is the seam the Android slice reuses on the main-process side of the Binder facade.
>
> **As-built (item 41, slice 3a of 3):** the RPC layer the Binder facade will carry is now JVM-tested pure Kotlin in `mcos-android-sdk` (`com.morainet.mcos.android.host.isolation`). Check 1 exists as `BinderIdentityPolicy` (caller-UID equality; mismatch → `PERMISSION_DENIED` with `details.reason = "plugin.identity_mismatch"`), enforced **first** in `IsolatedFacadeServer.handle` — on mismatch the host facade is never touched. Checks 2–3 run per-call in the same server, which composes the *same* decorators as the in-process Executor (`StampScopedNetService` over `SecretResolvingNetService`, `NamespacedSandbox` — item 41 made the latter two public in `ScopedFacade.kt` so the boundaries cannot drift); the plugin-process half is `IsolatedHostServicesProxy` (a `HostServices` facade that forwards `net`/`secureStore`/`sandbox`/`memory`/`clock` over the `IsolationChannel` wire codec, serves `json` locally, and answers `files`/`ui` with honest `UNAVAILABLE`), and `TransportIsolationHost` maps channel death to `PLUGIN_ERROR` rather than an exception (§8.1). **Item 42 (slice 3b part 1) adds the plugin-process execution half:** `IsolatedPluginRunner.serveInvoke` decodes the invocation, refuses invocations addressed to a different plugin id (the §8.2 identity reason checked on *both* sides of the wire), and rebuilds the `ExecutionContext` — forwarding the invocation's AuthStamp to the handler's facade **only when the stamp describes this very invocation** (`runId`+`commandId`+`pluginId` all match; otherwise stripped to null, so a replayed or cross-command stamp can never ride a proxy call into the gate), with the deadline enforced locally (`withTimeout` on the remaining budget) so a hung plugin cannot outlive its slot even if cancellation is never delivered. `IsolationEndToEndTest` composes the real Executor → transport host → runner → proxy → facade server over the host facade in one JVM process, with the main-process registry holding the plugin manifest only — six scenarios pin happy path (secret resolution + namespaced sandbox), confused-deputy out-of-scope URL, scopeless auto-approved stamp, process death, BUILTIN in-process, artifact round-trip. **Item 43 (slice 3b-final, code half) lands the byte transport:** the wire is `BinderWire` — one frame (`{"op","payload"}` as a single Parcel string) per Binder transaction, `CODE_INVOKE` main→plugin and `CODE_FACADE` plugin→main; `WireService` holds the pure serving cores both endpoints delegate to (never throwing — every failure is a framed error envelope); `FacadeBinderEndpoint` feeds `Binder.getCallingUid()` into check 1 (identity comes from the Binder kernel, never the wire); `IsolatedPluginProcessService` (`:mcos_plugin`, one bind = one plugin, masquerade-guarded manifest re-read) hosts the runner; `BinderIsolationHost` binds on first invoke with one `IsolatedFacadeServer` per plugin (§8.2 admission pinned at bind time) and re-binds after `linkToDeath`. A framed-wire E2E re-proves the whole §8.1-§8.3 chain through the exact channel objects the Binder endpoints sit behind. **Item 44 lands the activation seam:** `CompositionRoot.create(context, builtIns, processIsolation = false)` — flag on, the Executor dispatches every non-BUILTIN plugin through `BinderIsolationHost`, artifacts resolved at bind time by the JVM-tested `StagedArtifactResolver` (tamper-evident install records → staged `.mcos` files, `INSTALLED`-only — DISABLED plugins are refused there too); flag off (the default) production is byte-identical on the audited in-process fallback. Boundary stated on the flag: non-BUILTIN execution never runs in-process, and a bind failure is an honest `PLUGIN_ERROR`, never a silent fallback. **Item 45 closes the last boundary — manifest-only registration:** `McosPackage.readPluginManifest` decodes the full 04 §4 schema (an unknown `sideEffectClass` or a missing command `id` fails the decode fail-closed, never a silent downgrade to `read`); `CommandRegistry.registerManifest` shares the class-path registration core (descriptor enrichment, conflicts, aliases, plugin-permission merge) with every command holding `IsolationRequiredHandler` (an honest `PLUGIN_ERROR`/`isolation_required` reached only on a mis-wired host); `PluginLoader.loadManifest` runs the identical trust gate plus the id-masquerade guard; `PluginInstaller`'s `manifestDecoder` seam (wired by `CompositionRoot` under the flag, for installs AND restart rehydrate) registers without ever invoking `pluginFactory`. Under the flag the plugin's dex exists ONLY in `:mcos_plugin` — the main process carries none of the plugin's code, closing §8 as specified.

> **As-built (item 50, slice 4 of 4 — on-device Binder-kernel verification):** the full §8.1-§8.3 chain is now exercised on real hardware by `mcos-android-sdk`'s `BinderIsolationDeviceTest` (`androidTest`, runs via `connectedDebugAndroidTest` against an attached Android 10/API 29 handset — no CI emulator). Five end-to-end cases pin the layer the JVM `IsolationEndToEndTest` cannot reach: **BD1** install registers via the production `PluginInstaller`/`McosPackage.readPluginManifest` while the supplied `pluginFactory` is asserted **never** to run in the host process (manifest-only); **BD2** the first `echo` invoke produces a real `:mcos_plugin` process whose pid ≠ the host's; **BD3** `park`'s sandbox marker lands on disk through the §8.3-name-spaced facade hop and the marker carries the plugin-process pid; **BD4** mid-run `Process.killProcess(pid)` of the plugin process fails only the current run (`RunFailed("Isolated process call failed")`) while the host stays alive and serves a subsequent invoke; **BD5** a fresh invoke after death transparently rebinds through `linkToDeath` to a NEW plugin process (pid before ≠ pid after); **BD6** corrupting the staged artifact after a forced rebind produces an honest `RunFailed("failed to load in the plugin process")` instead of stale code. The PublisherKey the test injects probes the device's JCA at runtime (Ed25519 preferred, RSA-PSS-4096 fallback constructed identically to the production verifier — see `ArtifactVerifier.rsaPssSignature`, which this slice also retries under the digest-pinned `SHA256withRSA/PSS` name for OEM Android 10 builds that don't register the generic `RSASSA-PSS` alias), and the `.mcos` is signed with the same algorithm the verifier will use. **Honest boundaries (also recorded in `11`):** same-UID only (the `:mcos_plugin` split shares this app's UID, so foreign-uid rejection stays covered by the JVM `BinderIdentityPolicyTest` oracle and would need a second APK to exercise); the fixture's class is reachable from the instrumentation classloader by androidTest convention — what the suite proves is that *execution*, sandbox writes, and crash isolation happen in the SEPARATE plugin process (main-process dex-exclusivity is the manifest-only registration path's job, item 45); verified on a single Android 10 handset, not an emulator matrix. With item 50 the §8 surface is **closed for third-party hosts on this version line** — items 36-45 ship and are JVM-tested, the Binder kernel itself is now device-tested.
