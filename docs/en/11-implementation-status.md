...
| **Memory** | [07](./07-memory.md) | P2 | ✅ `memory/MemoryStore` — TTL, tags, fuzzy resolveRef + confidence, CREATED/UPDATED/CONFLICT write semantics, superseded history (`d549236`) |
...
| Memory | spec done | ✅ profile + remember | 🟡 fuzzy refs + conflict detection done; **episodic layer + cloud sync open** | cloud sync |
...
**Next up (suggested):** wire the Planner into the Android chat shell (needs an API key), then close the P1 security gaps (`{{secret}}` templates, crash quarantine), then the episodic Memory layer.
