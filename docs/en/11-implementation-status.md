...
**Next up (suggested):** Planner probes (§17 multi-provider probing: `LlmProviderRegistry.healthyProviders` exists; probing policy + UI surfacing remain), episodic-tier closure (§8: `EpisodicMemory` done; remaining §8.3 fuzzy references / named-entity merge) and a standalone `mcos-server` deployment (REST contract from `SyncBlobTransport` + auth).
