...
**Next up (suggested):** plugin sideload trust & signature verification ([09 §15](./09-marketplace.md)) — `TrustLevel` (SIGNED/DEBUG_SIDELOAD), `.mcos` signed package format & install verification, `disableSideload` enterprise policy enforced at the plugin loader (§13.2), Marketplace index & public signing (P3).
