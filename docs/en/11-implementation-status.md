...
**Next up (suggested):** enterprise policy allowlist/denylist ([08 §13](./08-security.md)) — policy rules for `decideConfirmation`/`decideEgress` (domain/IP denylist, command/argument allowlist, `blockingPrefixes`/`allowBlockingWorkflows`), policy-file hot reload and hit recording in the AuditLog.
