...
> ✅ **实现状态：** 下述运行时管线**已实现**——Parser、Registry、Executor（含 §8 调度器：四条优先级通道、§8.2 并发上限、§8.4 背压、§8.5 经 workflow `requiresDevices` 的设备串行化）、Workflow Engine、Event Bus、Memory 与 Audit 位于 `mcos-runtime-core`；Permission Kernel、egress 策略与审计落盘位于 `mcos-security`；薄门面 `McosRuntime` 位于 `mcos-runtime`。已知 🟡 差距：设备互斥键按字面取值（`x-mcos-ref` 自然语言值未经 Memory 规范化——完整 Stage-4 Expand 尚未构建）；调度器热调参（§8 运行时配置）仅限 Builder 构建期；第三方进程隔离尚未强制（P3，见 [08-security.md](./08-security.md) §8）。逐子系统状态见 [11-implementation-status.md](./11-implementation-status.md) §3。
