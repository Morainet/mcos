...
- [ ] §17.0 中每个表面都有一条**会随该表面变化而失败**的一致性用例 —— `mcos-conformance` 即回归门（10 §6.4、09 §5.1），`./gradlew :mcos-conformance:conformance` 在 CI 中运行。
- [ ] 黄金 DSL↔IR 用例（`docs/fixtures/`，8 例）在一个完整发布周期内保持全绿且未改动。
- [ ] 可选能力模式完整：**每一个**宿主可能缺失的能力都是可空类型，并配有在案的 `UNAVAILABLE` 降级路径 —— 树中不存在任何假成功。
- [ ] 错误码表（02 §15）没有未分配或有歧义的编码；每个编码至少有一条测试断言它。
- [ ] 协议版本化（02 §14）被记录为**唯一**的变更机制，不留静默演进通道。
...
**剩余项已收口（2026-09-10），门尚未关闭。** 卡住冻结的两项契约表面工作已完成（[11 item 59](./11-implementation-status.md)）：设备互斥键规范化（03 §8.5——别名现解析为 canonical Memory id,尽力而为并记录了字面量回退）与隔离进程的能力缺口（04 §6.1——代理的防御分支现在响亮地失败；跨 Binder 边界无假成功）。门关闭前剩下的是验证而非代码：§17.1 的错误码覆盖断言与逐表面 conformance 用例。
