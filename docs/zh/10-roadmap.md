...
**提议中。** 各契约表面的剩余工作记录在 [11-implementation-status.md](./11-implementation-status.md) 的"下一步"清单里 —— 主要是设备互斥键规范化（Runtime Semantics）与隔离进程的能力缺口（Plugin Contract）。那份清单收口，这个门就关闭。
