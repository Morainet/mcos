...
## 17. Kernel 1.0 稳定门（Stabilization Gate）

> **来源：** issue #14 —— 一份外部架构评审（2026-09-08）。其核心发现是：**功能增长速度超过了核心模型的稳定速度**，而决定项目成立与否的只有三件事 —— **Command Protocol、Runtime Semantics、Plugin Contract**。本节把这一发现落地为一个显式的、跨阶段的里程碑。评审的其余建议本就与已交付状态一致（一致性门禁、插件生命周期、进程隔离、多厂商 planner、MCP 仅作 adapter —— 见 [11 §6](./11-implementation-status.md)）；当时**尚未显式化**的只有生态成熟度与这个门。

### 17.0 冻结什么

Kernel 1.0 **不是**一个功能版本。它是下面三份契约停止变动的时刻 —— 此后插件作者与宿主移植方可以基于 MCOS 构建，而不必在每次合并后重新验证。

| 契约 | 表面 | "冻结"的含义 |
|---|---|---|
| **Command Protocol** | DSL 文法、IR 形状、错误码表、`dslVersion`（02 §6、§7、§14、§15） | 形状变更必须伴随新的 `dslVersion` 次版本号 + 迁移说明。静默的形状变化是 bug，不是重构。 |
| **Runtime Semantics** | Executor 阶段顺序、权限/出网决策、调度通道与取消、审计记录形状（03 §8、§9、§13） | 可观测行为由测试钉死。调整阶段顺序必须当作规范修订来论证，绝不能包装成"优化"。 |
| **Plugin Contract** | `McosPlugin` / `CommandHandler` / `HostServices` / `ExecutionContext` / manifest schema（04 §3–§6） | 只增不改 —— 新的可选能力、新的可选 manifest 字段。破坏性变更需要主版本号提升。 |

**明确不在冻结范围内：** 叠在内核**之上**的一切 —— 市场、recipe 商店、planner 供应商、Memory、宿主移植（Android/iOS/桌面）与新插件。它们继续演进，脚下的内核不动。

### 17.1 退出标准

- [ ] §17.0 中每个表面都有一条**会随该表面变化而失败**的一致性用例 —— Command Protocol 表面由 `dsl`/`ir`/`kernel` 套件覆盖、Plugin Contract 表面由 `manifest`/`trust` 覆盖；Runtime Semantics 表面（Executor 阶段顺序、调度通道）由 JVM 单测钉住，但尚无专门的一致性用例。（`mcos-conformance` 即回归门——10 §6.4、09 §5.1；6 套件 / 69 用例。）
...
- [x] 协议版本化（02 §14）被记录为**唯一**的变更机制，不留静默演进通道 —— `kernel` 套件拒绝未来 `dslVersion` 头。*（2026-09-10，item 60。）*

### 17.2 门开启期间的治理

- **文档先行**（§1.1 原则 5）：冻结之后，任何行为变更都必须随同一个 PR 交付规范修订**与** CHANGELOG 条目 —— 代码不得静默偏离。
- **功能暂缓是有边界的**：新增*协议表面*的工作（新的 IR 节点、新的 Executor 阶段、新的必填 manifest 字段）推迟到门关闭之后；生态工作 —— 市场、第三方插件、宿主移植、部署 —— 明确**不受阻塞**，因为它们不触碰被冻结的表面。

### 17.3 状态

**剩余项已收口（2026-09-10）；§17.1 五条标准已满足三条。** 卡住冻结的两项契约表面工作已完成（[11 item 59](./11-implementation-status.md)）：设备互斥键规范化（03 §8.5）与隔离进程的能力缺口（04 §6.1——跨 Binder 边界无假成功）。验证的一半也已到位（[11 item 60](./11-implementation-status.md)）：`kernel` 一致性套件钉住错误码词汇表、retryable 集与逐码测试覆盖登记表，并拒绝未来 `dslVersion`。门关闭前还剩：专门的 Runtime Semantics 一致性表面（Executor 阶段顺序、调度通道——目前仅由 JVM 单测钉住），以及黄金 fixtures 在一个完整发布周期内保持全绿。
