...
工作流引擎是 **Runtime 的一个子组件**（[01 §6.3](./01-architecture.md)），位于包 `com.mcos.runtime.workflow`（[03 §3](./03-runtime.md)）。它**不是** [01 §9.2](./01-architecture.md) 中定义的 10 个流水线阶段之一，而是位于流水线**之上**。每个工作流步骤都是一次单独的命令调用，会重新进入流水线。
