...
| 场景 | 动作 | 客户端影响 |
|----------|--------|---------------|
| 例行轮换（发布者主动选择） | 发布者生成新密钥，以 `rotatedFrom: oldKeyId` 注册——该链接由**新**密钥携带——再用它签名下一个发布 | 旧密钥保持 `ACTIVE` 直到发布者主动退役；90 天是**建议**的重叠期、不是强制定时器，且轮换本身不强制禁用任何东西 |
...
**宽限期理由：** 例行轮换不得破坏已安装的插件。建议的 90 天重叠期让发布者能用新密钥对现有版本重新签名并推送更新，之后旧密钥才会被完全吊销。该重叠期是**建议性的**：市场不跑定时器——旧密钥会保持 `ACTIVE` 直到发布者调用撤销端点，因此一个永不退役的发布者会让它无限期可验签。
