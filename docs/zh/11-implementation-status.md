...
**下一步（建议）：** Memory 云端 server（Phase 3 blob 加密 + E2E）、情景层（§8）与 Planner 探测（§17 多 provider 探测）。
