...
| **Planner 桥** | [06](./06-agent.md) | P1（单一 provider） | ✅ `llm/` LlmPlanner + OpenAiLlmProvider + ChatOrchestrator；未接入 Android UI |
...
>
> **测试基线（2026-08-12）：** 全模块 361 个测试——parser fixture、executor、permission、audit、workflow（W1-W6）、event bus（8）、memory（M1-M33）、插件、Android。
...
10. ✅ **一个 LLM provider**——`LlmPlanner` + OpenAI provider；话语 → 单条命令 / 短序列。按 [06](./06-agent.md) §3。Android 聊天 UI 未连接。

**下一步（建议）：** 把 Planner 接入 Android 聊天外壳（需 API key），然后补齐 P1 安全缺口（`{{secret}}` 模板、crash 隔离），再做 Memory 情景层。
