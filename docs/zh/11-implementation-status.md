...
**下一步（建议）：** 插件侧载信任与签名验证（[09 §15](./09-marketplace.md)）——`TrustLevel`（SIGNED/DEBUG_SIDELOAD）、`.mcos` 签名包格式与安装验证、`disableSideload` 企业策略在插件加载器处落地（§13.2）、Marketplace 索引与公共签名（P3）。
