...
**下一步（建议）：** 独立 `mcos-server` 仓库部署（对接 `SyncBlobTransport` 的 REST 契约 + 认证）、情景层（§8：`EpisodicMemory` 已实现，剩余 §8.3 模糊引用/命名实体合并）与 Planner 探测（§17 多 provider 探测）。
