...
**下一步（建议）：** 企业策略允许/拒绝名单（[08 §13](./08-security.md)）——`decideConfirmation`/`decideEgress` 的策略规则（域名/IP 黑名单、命令/参数白名单、`blockingPrefixes`/`allowBlockingWorkflows`）、策略文件热加载与命中 AuditLog 记录。
