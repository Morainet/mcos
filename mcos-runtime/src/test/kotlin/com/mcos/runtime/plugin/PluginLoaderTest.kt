...
import com.mcos.runtime.security.KeyStatus
...
class PluginLoaderTest {
    ...
    fun `T6-registered handler resolves through the registry`() {
        ...
        val plugin = object : McosPlugin {
            override val manifest = PluginManifest(
                ...
                description = "d",
                provider = ProviderInfo("Test", "https://test.local"),
                entry = "com.mcos.plugin.test.Hello",
                commands = listOf(
                    com.mcos.sdk.CommandManifestEntry(
                        id = "hello.greet",
                        version = "1",
                        title = "Greet",
                        description = "g",
                        sideEffectClass = com.mcos.sdk.SideEffectClass.read,
                    ),
                ),
            )
            override suspend fun onLoad(services: HostServices) {}
    ...
    private fun sign(pair: java.security.KeyPair, payload: ByteArray): com.mcos.runtime.security.ArtifactSignature {
        val signer = java.security.Signature.getInstance("Ed25519")
        signer.initSign(pair.private)
        signer.update(payload)
        val bytes = signer.sign()
        return com.mcos.runtime.security.ArtifactSignature(
