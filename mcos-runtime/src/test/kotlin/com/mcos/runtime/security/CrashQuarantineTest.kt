...
class CrashQuarantineTest {
    ...
    fun `fewer than threshold crashes does not quarantine`() {
        val q = CrashQuarantine(windowMs = 60_000, threshold = 3)
    ...
    fun `crashes outside the window do not accumulate`() {
        var now = 1_000L
        val q = CrashQuarantine(windowMs = 60_000, threshold = 3, timeSource = { now })
        q.recordCrash("p", "t1")
    ...
    fun `already quarantined plugin does not re-trigger`() {
        val q = CrashQuarantine(windowMs = 60_000, threshold = 2)
        q.recordCrash("p", "t1")
