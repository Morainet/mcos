...
组装成单一入口对象。**本模块只做组装与协调，不实现子系统**（main 仅 4 个文件）。
...
└── api/
    ├── McosRuntime.kt            # 门面 + Builder（实现 core.api.RuntimeGateway）
    ├── RunManager.kt             # internal：in-flight run 生命周期（SupervisorJob 防泄漏）
...
| API | 说明 |
|-----|------|
| `execute(request): ExecuteHandle` | 解析 payload → Plan → 启动 run；解析失败同步返回 FAILED handle |
...
- `Executor`（7 阶段）、`DslParser`、`CommandRegistry`、`EventBus`、`WorkflowEngine`、
