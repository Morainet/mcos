 ...
 * - Optional durable grant table via [GrantStore] (replayed on
 *   construction, written through on every mutation; session grants are
 *   filtered out of snapshots so they die with the process)
 ...
 *
 * A corrupt or tampered store fails **closed**: [GrantStore.load] yields
 * null and the kernel starts empty (denials), never a crash and never a
 * resurrected grant.
 *
...
class DefaultPermissionKernel(
    ...
    init {
        // Replay durable consent state. A missing/corrupt/tampered store
    ...
    /**
     * Mirror the durable consent state to [grantStore] (write-through).
     *
     * Session-scoped grants are filtered out — they expire with the process
     * by contract and must never be resurrectable from disk.
     */
    private fun persist() {
        if (grantStore === NullGrantStore) return
        val sessionKeys = sessionGrants.toSet()
        val durable = buildMap<String, List<String>> {
            for ((pid, perms) in grants) {
                synchronized(perms) {
                    val durablePerms = perms.filter { "$pid:$it" !in sessionKeys }
                    if (durablePerms.isNotEmpty()) put(pid, durablePerms)
                }
            }
        }
        grantStore.save(
            GrantSnapshot(
                grants = durable,
                autoApprove = autoApprove.toList(),
