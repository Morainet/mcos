...
/**
 * End-to-end orchestrator that combines [LlmPlanner] with a [RuntimeGateway]
 * to implement the full "chat to execution" pipeline.
 *
 * The gateway port (defined in `mcos-runtime-core`, canonically implemented by
 * the facade's `McosRuntime`) keeps this module decoupled from the
 * `mcos-runtime` facade — llm and the facade are sibling clients of the kernel.
 *
 * ## Pipeline
 *
 * ```
 * NL input → LlmPlanner.plan() → parsed Commands → [PromptInjectionDetector] → RuntimeGateway.execute() → ChatResult
 * ```
 *
 * ## Usage
 *
 ...
 * @param runtime The [RuntimeGateway] for executing the generated commands.
...
data class ChatResult(
    ...
    /**
     * Per-command execution results from [RuntimeGateway.execute].
